...
<style>
    ...
    hr { border-color: rgba(255, 107, 53, 0.2); }
    .footer {
        text-align: center; color: #666; font-size: 0.8rem;
        padding: 20px 0; border-top: 1px solid rgba(255, 107, 53, 0.1); margin-top: 40px;
    }
    ...
    .summer-badge { background: rgba(255,107,53,0.15); color: #FF6B35; }
    .winter-badge { background: rgba(96,165,250,0.15); color: #60A5FA; }
</style>
...
if user_menu == 'Medal Tally':
    ...
    if selected_year == 'Overall' and selected_country == 'Overall':
        ...
        st.title(f'Medal Tally of {selected_country} — All Years')
    else:
...
if user_menu == 'Overall Analysis':
    ...
    with col1: st.metric("Editions", f"{editions:,}")
    with col2: st.metric("Host Cities", f"{cities:,}")
    ...
    if len(all_years) >= 2:
        yr_min, yr_max = st.slider(
            'Filter Year Range', min_value=int(all_years[0]),
    ...
    fig = px.line(nations_ot, x='Edition', y='Number of Nations',
                  title='Participating Nations Over Time')
    styled_line(fig, ACCENT)
    st.plotly_chart(fig, use_container_width=True)

    # Events over time
    events_ot = helper.data_over_time(df_filtered, 'Event')
    fig = px.line(events_ot, x='Edition', y='Number of Events',
                  title='Events Over Time')
    styled_line(fig, TEAL)
    st.plotly_chart(fig, use_container_width=True)

    # Athletes over time
    athletes_ot = helper.data_over_time(df_filtered, 'Name')
    fig = px.line(athletes_ot, x='Edition', y='Number of Athletes',
                  title='Athletes Over Time')
    styled_line(fig, YELLOW)
    st.plotly_chart(fig, use_container_width=True)
    ...
    with st.spinner("Rendering heatmap..."):
        ...
        styled_heatmap(fig, len(heatmap_data))
...
if user_menu == 'Country-wise Analysis':
    ...
    selected_country = st.sidebar.selectbox('Select Country', country_filtered)
    ...
    if pt.empty:
        ...
        with st.spinner("Rendering heatmap..."):
            pt = pt.astype(int)
            fig = px.imshow(pt, text_auto=True, aspect='auto',
                            color_continuous_scale='OrRd')
            styled_heatmap(fig, len(pt), row_height=28)
            fig.update_layout(xaxis_title='Year', yaxis_title='Sport')
            st.plotly_chart(fig, use_container_width=True)
...
if user_menu == 'Athlete wise Analysis':
    ...
    fig = px.histogram(age_df, x='Age', nbins=50,
                       title='Age Distribution of All Athletes',
    ...
    fig = px.box(medal_age_df, x='Medal', y='Age',
    ...
    if not bmi_stats.empty:
        ...
        with col1:
            fig = px.bar(bmi_stats.head(20), x='Avg BMI', y='Sport',
                         orientation='h', title='Average BMI by Sport (Top 20)',
                         color='Avg BMI', color_continuous_scale='OrRd')
        ...
        fig = px.histogram(bmi_dist, x='BMI', nbins=60, color='Sex',
                           title=f'BMI Distribution — {sport_for_bmi}',
                           color_discrete_map=GENDER_COLORS, barmode='overlay',
                           opacity=0.7)
        style_chart(fig)
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.scatter(phys_df, x='Height', y='Weight', color='Medal',
                     hover_data=['Name', 'Sport'],
                     title='Height vs Weight of Athletes',
                     color_discrete_map=MEDAL_COLORS, opacity=0.5)
    style_chart(fig)
    st.plotly_chart(fig, use_container_width=True)

    # Gender participation
    st.subheader('Gender Participation Over Time')
    gender_df = df.drop_duplicates(subset=['Year', 'Name', 'Sex'])
    gender_counts = gender_df.groupby(['Year', 'Sex']).size().reset_index(name='Count')
    fig = px.line(gender_counts, x='Year', y='Count', color='Sex',
    ...
    min_ed = st.slider('Minimum Olympic editions with medals', 2, 6, 3, key='repeat_slider')
    ...
    if repeat_df.empty:
        ...
        st.dataframe(repeat_df, use_container_width=True, height=500)
...
if user_menu == '🌍 Medal Map':
    ...
    fig = px.choropleth(
        ...
        color_continuous_scale='OrRd',
        title=f'{medal_type} Medals by Country',
    )
    fig.update_layout(
        geo=dict(
            showframe=False, showcoastlines=True,
            coastlinecolor='#333', landcolor='#1A1F2E',
            bgcolor='rgba(0,0,0,0)', lakecolor='#0E1117',
        ),
    ...
    st.dataframe(
        choro_data[['region', 'Gold', 'Silver', 'Bronze', 'Total']].head(30),
...
if user_menu == '⚔️ Country Comparison':
    ...
    with col2:
        c2 = st.selectbox('Country 2', countries_list,
                          index=countries_list.index('China') if 'China' in countries_list else 1)
    ...
    if c1 == c2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=merged['Year'], y=merged[c1], name=c1,
                                 line=dict(color=ACCENT, width=2.5)))
        fig.add_trace(go.Scatter(x=merged['Year'], y=merged[c2], name=c2,
        ...
        with col1_:
            ...
            st.write(', '.join(sorted(shared)) if shared else 'None')
        with col2_:
            st.markdown(f'**🔶 Only {c1} ({len(only1)})**')
            st.write(', '.join(sorted(only1)) if only1 else 'None')
        with col3_:
            st.markdown(f'**🔷 Only {c2} ({len(only2)})**')
            st.write(', '.join(sorted(only2)) if only2 else 'None')

        st.divider()

        # Top athletes side by side
        st.subheader('Top Athletes')
        col1_, col2_ = st.columns(2)
        with col1_:
            st.markdown(f'**{c1}**')
            st.table(helper.most_successful(df, 'Overall', country=c1, top_n=10))
        with col2_:
            st.markdown(f'**{c2}**')
            st.table(helper.most_successful(df, 'Overall', country=c2, top_n=10))
...
if user_menu == '☀️❄️ Summer vs Winter':
    ...
    with col_summer:
        st.markdown('<span class="season-badge summer-badge">☀️ SUMMER</span>',
                    unsafe_allow_html=True)
        cols = st.columns(3)
        for i, m in enumerate(metrics_list):
            with cols[i % 3]:
                st.metric(m, f"{sc['Summer'][m]:,}")

    with col_winter:
        st.markdown('<span class="season-badge winter-badge">❄️ WINTER</span>',
                    unsafe_allow_html=True)
        cols = st.columns(3)
        for i, m in enumerate(metrics_list):
            with cols[i % 3]:
                st.metric(m, f"{sc['Winter'][m]:,}")
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(y=dom['region'], x=dom['Summer'], name='Summer',
                         orientation='h', marker_color=SEASON_COLORS['Summer']))
    fig.add_trace(go.Bar(y=dom['region'], x=dom['Winter'], name='Winter',
                         orientation='h', marker_color=SEASON_COLORS['Winter']))
    fig.update_layout(barmode='stack', yaxis=dict(autorange='reversed'),
    ...
    # Sports split
    st.subheader('Sports by Season')
    summer_sports = set(df_all[df_all['Season'] == 'Summer']['Sport'].unique())
    winter_sports = set(df_all[df_all['Season'] == 'Winter']['Sport'].unique())
    both_sports = summer_sports & winter_sports

    col1, col2, col3 = st.columns(3)
    with col1:
        st.markdown(f'**☀️ Summer Only ({len(summer_sports - winter_sports)})**')
        st.write(', '.join(sorted(summer_sports - winter_sports)))
    with col2:
        st.markdown(f'**🔄 Both Seasons ({len(both_sports)})**')
        st.write(', '.join(sorted(both_sports)) if both_sports else 'None')
    with col3:
        st.markdown(f'**❄️ Winter Only ({len(winter_sports - summer_sports)})**')
        st.write(', '.join(sorted(winter_sports - summer_sports)))
    ...
    if cross.empty:
        ...
        medalists = cross[cross['Medals'] > 0]
        if not medalists.empty:
            st.markdown(f'**🏅 {len(medalists)} of them won medals!**')
...
st.markdown("""
<div class="footer">
    📊 Data source: <a href="https://www.kaggle.com/datasets/heesoo37/120-years-of-olympic-history-athletes-and-results" target="_blank">
    120 Years of Olympic History</a> (Kaggle) &nbsp;|&nbsp;
